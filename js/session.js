...
function ncProteger(cargosPermitidos=[]) {
  const token=ncGetToken(), u=ncGetSesion();
  if(!token||!u){window.location.href='index.html';return false;}
  if(cargosPermitidos.length>0){
    const tieneAcceso=cargosPermitidos.some(c=>ncTieneAcceso(c));
    if(!tieneAcceso){window.location.href='index.html';return false;}
  }
  ncAplicarSesion();
  return true;
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const protecciones = {
    ...
    'programacion.html':   ['jefatura','programacion'],
    'supgrabaciones.html': ['jefatura','supgrabaciones'],
  };

  const cargos=protecciones[pagina];
  if(cargos) ncProteger(cargos); else ncProteger();
});
