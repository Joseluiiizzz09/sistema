...
const ESTADOS = [
  { id:'ejecucion', label:'EN EJECUCIÓN',      cls:'bs-ejec',    fila:'fila-ejec',    color:'#00cc00' },
  { id:'instalado', label:'INSTALADO',          cls:'bs-inst',    fila:'fila-inst',    color:'#00ccff' },
  { id:'caida',     label:'CAÍDA',              cls:'bs-caida',   fila:'fila-caida',   color:'#ff3333' },
  ...
  { id:'tecnico',   label:'TÉCNICOS EN CASA',   cls:'bs-tecnico', fila:'fila-tecnico', color:'#ff66cc' },
];

const MOTIVOS_CAIDA = ['FRAUDE','INFRAESTRUCTURA','DUPLICIDAD','SERVICIO ACTIVO','ANTIFRAUDE','TELÉFONO EN BL','PLANO FRAUDE','EXCESO DE ACOMETIDA','EDIFICIO NO LIBERADO','RIESGO CREDITICIO','OTRO'];
...
const RESULTADOS    = ['Contactado — conforme','Contactado — con problema','No contesta','Buzón de voz','Número equivocado','Solicita rellamada','SE LEVANTÓ','MASIVO ENVIADO','DERIVADO A GRABAR','DERIVADO A AGILIZAR','En Agenda'];
...
function mapearEstado(e){
  const m={instalado:'instalado',programado:'ejecucion',validado:'ejecucion',caida:'caida',observado:'novalidado',venta:'novalidado',pendiente:'novalidado'};
  if((e||'').toLowerCase().includes('tecnico')||e==='tecnico') return 'tecnico';
  return m[e]||'novalidado';
}
...
function actualizarKpis(){
  ...
  document.getElementById('cnt-todos').textContent=ventas.length;
}
...
function renderTabla(){
  ...
  document.getElementById('pagInfoTop').textContent=total?`Mostrando ${ini+1}–${fin} de ${total}`:'';
  ...
  tbody.innerHTML=pag.map(v=>{
    ...
    const proxV=v._proxSeg;
    const proxLabel=proxV?(proxV<fechaHoy()?`<span style="color:#dc2626;font-weight:700;font-size:10px">⚠️${formatF(proxV)}</span>`:`<span style="color:#16a34a;font-size:10px;font-weight:600">${formatF(proxV)}</span>`):'—';
    return `<tr class="${est.fila}">
      <td style="text-align:center;vertical-align:middle;">
        <div class="acciones-cell">
          <button class="btn-acc btn-acc-estado" onclick="abrirModalEstado(${v.id})" title="Cambiar estado">🔄</button>
          <button class="btn-acc btn-acc-obs"    onclick="abrirModalObs(${v.id})"    title="Registrar llamada">💬</button>
          <button class="btn-acc btn-acc-agenda" onclick="abrirModalAgenda(${v.id})" title="Agendar">📅</button>
          <button class="btn-acc" style="border-color:#c4b5fd;background:#faf5ff;font-size:13px;" onclick="hAbrir(${v.id},{nombre:'${(v.nombreApellidos||'').replace(/'/g,'').substring(0,30)}',dni:'${v.dni||''}',n1:'${v.telefonoContacto||''}'})" title="Historial">📋</button>
        </div>
      </td>
      <td class="td-estado"><span class="badge-seg ${est.cls}" onclick="abrirModalEstado(${v.id})">${est.label}</span></td>
...
async function guardarEstado(){
  ...
  await guardarVentaBackend(v.id, { estado: v._estadoSeg==='instalado'?'instalado':v._estadoSeg==='caida'?'caida':'validado', obs_backoffice: v.obsBackOffice||'' });
...
async function guardarObservacion(){
  ...
  if(!obs){ toast('⚠️ Escribe una observación'); return; }
...
async function guardarAgenda(){
  ...
  if(!f){ toast('⚠️ Selecciona una fecha'); return; }
  v._proxSeg=f;
  const nota=document.getElementById('ag_nota').value.trim();
  if(nota){ if(!v._historial)v._historial=[]; v._historial.push({fecha:fechaHoy(),hora:horaAhora(),user:usuarioActual,resultado:'Agendado',obs:`Próx: ${formatF(f)}. ${nota}`}); }
...
function exportarCSV(){
  ...
  const h=['Estado','Fecha','Nombre','DNI','Distrito','Dirección','Vendedor','Supervisor','Tramo','Comentario','Motivo','NODECON','EFONODEC','Tel. Contacto'];
