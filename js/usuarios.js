...
function renderTabla(lista) {
  ...
  if (!data.length) {
    tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state">
      ...
      <div class="empty-sub">Crea el primer usuario con el botón "Nuevo Usuario"</div>
    </div></td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = data.map((u, i) => {
    ...
    const estadoBadge = u.activo
      ? `<span class="badge-activo"><span class="dot dot-green"></span>Activo</span>`
      : `<span class="badge-inactivo"><span class="dot dot-red"></span>Inactivo</span>`;
    const btnToggle = u.activo
      ? `<button class="btn-toggle btn-toggle-on" onclick="toggleEstado(${u.id}, false)">Desactivar</button>`
      : `<button class="btn-toggle btn-toggle-off" onclick="toggleEstado(${u.id}, true)">Activar</button>`;
    return `<tr>
      <td style="color:#9ca3af;font-size:12px;">${i+1}</td>
      <td>
        <div style="font-weight:700;font-size:13px;color:#111827;">${u.nombre}</div>
        <div style="font-size:11px;color:#9ca3af;">Género: ${u.genero === 'F' ? 'Femenino' : 'Masculino'}</div>
      </td>
      <td style="font-family:monospace;font-size:13px;font-weight:600;color:#374151;">@${u.usuario}</td>
      <td><span class="badge-cargo cargo-${u.cargo}">${u.cargo}</span></td>
      <td style="font-size:13px;color:#374151;">${u.sala || '—'}</td>
      <td>${estadoBadge}</td>
      ...
      <td>
        <div class="acciones-fila">
          ...
          <button class="btn-eliminar-fila" onclick="abrirModalEliminar(${u.id}, '${u.nombre.replace(/'/g,"\\'")}')">🗑️</button>
        </div>
      </td>
    </tr>`;
  }).join('');
...
function abrirModalEditar(id) {
  ...
  document.getElementById('modalTitulo').textContent = `Editar — ${u.nombre}`;
  ...
  try {
    ...
    if (editandoId) {
      ...
      const body = {
        ...
        sala:   document.getElementById('u_sala').value,
        genero: document.getElementById('u_genero').value,
      };
      ...
      await fetch(`${API}/usuarios/${editandoId}/estado`, {
        method: 'PATCH', headers: ncHeaders(),
        body: JSON.stringify({ activo: document.getElementById('u_activo').checked }),
      });
      ...
      res  = await fetch(`${API}/usuarios`, {
        ...
        body: JSON.stringify({
          nombre, usuario, password: pass, cargo,
          sala:   document.getElementById('u_sala').value,
          genero: document.getElementById('u_genero').value,
...
async function confirmarEliminar() {
  ...
  try {
    const res  = await fetch(`${API}/usuarios/${eliminandoId}`, {
      method: 'DELETE', headers: ncHeaders(),
    });
    const data = await res.json();
    if (!data.ok) { mostrarToast('❌ ' + data.mensaje); btn.disabled=false; btn.textContent='🗑️ Sí, eliminar'; return; }
    ...
    mostrarToast('❌ Error conectando al servidor');
    btn.disabled=false; btn.textContent='🗑️ Sí, eliminar';
  }
}
